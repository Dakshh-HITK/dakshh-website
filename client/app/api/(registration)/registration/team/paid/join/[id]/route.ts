...
export async function POST(
  ...
  try {
    ...
    if (String(event._id) !== id) {
      return NextResponse.json(
        { error: "Team code is for a different event" },
        { status: 400 },
      );
    }
    ...
    if (!event.isPaidEvent) {
      return NextResponse.json(
        { error: "This event is a free event" },
        { status: 400 },
      );
    }
    ...
    await Promise.all([newRegistration.save(), targetTeam.save()]);

    return NextResponse.json(
      {
        message: `Successfully joined team for ${event.eventName}! Complete payment to confirm your registration.`,
        isVerified: true,
    ...
    console.error("Joining Paid Team Error:", error);
    return NextResponse.json(
