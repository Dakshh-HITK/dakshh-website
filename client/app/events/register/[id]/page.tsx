...
const HiddenRegisterPage = () => {
  ...
  const id = useMemo(() => {
    ...
    if (pathname?.startsWith("/events/register/")) {
      const segments = pathname.split("/").filter(Boolean);
      const registerIdx = segments.indexOf("register");
      if (registerIdx !== -1 && segments[registerIdx + 1])
        return segments[registerIdx + 1];
    }
    return null;
  }, [params?.id, pathname]);
  ...
  const registerUrl = `/events/register/${id}`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (eventRes.status === 401) {
        redirecting = true;
        const callbackUrl = encodeURIComponent(registerUrl);
  ...
  const handleRegistrationResponse = async (registration: Response) => {
    ...
    if (registration.status === 401) {
      redirecting = true;
      const callbackUrl = encodeURIComponent(registerUrl);
      const message = encodeURIComponent("Please log in to register");
  ...
  if (!isHiddenRegisterEvent(event.eventName)) {
    return (
      <div className="min-h-screen w-full bg-black text-white flex items-center justify-center">
        <Navbar />
        <p className="text-white/80">Event not available for registration.</p>
      </div>
    );
  }
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            <HandDrawnCard className="min-h-[400px] flex flex-col p-6 sm:p-10 relative group">
              ...
              <div className="mt-8 sticky bottom-0 pt-4 bg-linear-to-t from-black/90 to-transparent space-y-3">
                {/* Hidden register page: always show registration UI, never "Registration Closed" */}
                {event.isTeamEvent ? (
...
export default HiddenRegisterPage;
